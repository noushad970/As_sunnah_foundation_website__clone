...
// Handle new ongoing project addition
$add_message = '';
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_project'])) {
    ...
    if ($title === '') {
        ...
        $stmt = $conn->prepare("INSERT INTO ongoing_project (title, description) VALUES (?, ?)");
        $stmt->bind_param("ss", $title, $description);
        if ($stmt->execute()) {
            $add_message = "✅ Ongoing project added successfully.";
        } else {
            $add_message = "❌ Error adding ongoing project.";
        }
        $stmt->close();
    }
}
...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM ongoing_project WHERE id = ?");
    $stmt->bind_param("i", $delete_id);
    $stmt->execute();
    $stmt->close();
    header("Location: ongoing_project_admin.php"); // refresh to avoid resubmission
    exit();
}

// Fetch all ongoing projects
$result = $conn->query("SELECT * FROM ongoing_project ORDER BY id DESC");
?>
...
<div class="container">
  ...
  <table>
    ...
    <tbody>
      ...
      <?php else: ?>
        <tr>
          <td colspan="4" style="text-align:center;">No ongoing projects found.</td>
        </tr>
      <?php endif; ?>
    </tbody>
  </table>
